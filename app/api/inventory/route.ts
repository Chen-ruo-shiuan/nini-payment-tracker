...
export async function POST(req: NextRequest) {
  ...
  const { name, category, unit, spec, cost_price, selling_price, initial_qty, low_stock_threshold, note } = body
  ...
  const result = db.prepare(`
    INSERT INTO inventory_items (name, category, unit, spec, cost_price, selling_price, current_qty, low_stock_threshold, note)
    VALUES (@name, @category, @unit, @spec, @cost_price, @selling_price, @current_qty, @low_stock_threshold, @note)
  `).run({
    name: name.trim(),
